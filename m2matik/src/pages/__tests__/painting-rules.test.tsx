...
function calcPaintingScenario(
  ...
  const base = baseTotal(
    MALING,
    area * (coveragePct / 100),
    factorKind,
    "maling"
  );
  const panels = includePanels
    ? baseTotal(PANELS, area, "normal", "højePaneler")
    : 0;
...
describe("painting business rules", () => {
  it("changing painting % changes only painting", () => {
    ...
    const onlyPaintDiff =
      baseTotal(MALING, area * 0.8, "normal", "maling") -
      baseTotal(MALING, area * 0.5, "normal", "maling");
  ...
  it("panels/stuk change only with their own area/quality", () => {
    ...
    expect(a - b).toBe(
      baseTotal(MALING, area, "normal", "maling") -
        baseTotal(MALING, area * 0.6, "normal", "maling")
    );
  });
});
