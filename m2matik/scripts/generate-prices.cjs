...
function toJsonKey(n) {
  ...
  switch (n) {
    case 'maling': return 'maling';
    case 'badevrelse': return 'badeværelse';
    case 'kkken': return 'køkken';
    case 'elektriker': return 'elektriker';
  case 'dreogvinduer': return 'døreOgVinduer';
    case 'fladt': return 'tag';
    case 'terasse':
    case 'terrasse': return 'terrasse';
    case 'gulv':
    case 'gulve': return 'gulv';
...
function main() {
  ...
  for (let i = 0; i < A.length; i++) {
    const c0 = norm(A[i]?.[0] ?? '');
    if (c0 === norm('arbejdstype')) { headerRow = i; break; }
  }
  if (headerRow < 0) {
  ...
  const m2Idx = colIndex(['m2 pris','m2pris','pris pr m2','pris/m2','kr/m2','krprm2']);
  const lavIdx = colIndex(['laveste kvalitet faktor','laveste faktor','lav','low']);
  const hojIdx = colIndex(['højeste kvalitet faktor','hojeste kvalitet faktor','høj','hoj','high']);
  ...
  for (let i = headerRow + 1; i < A.length; i++) {
    ...
    if (n === norm('træværk') || n === 'traevrk' || n === 'traevrk' || n === 'traevaerk') {
      if (startVal > 0) pushExtra('maling', { name: raw + ' (fast)', kind: 'fixed', amount: startVal });
      if (m2Val > 0) pushExtra('maling', { name: raw + ' (pr. m²)', kind: 'per_m2', amount: m2Val });
      continue;
    }
    if (n === 'stuk') {
      if (startVal > 0) pushExtra('maling', { name: raw + ' (fast)', kind: 'fixed', amount: startVal });
    ...
    if (n.includes('placering') && (n.includes('bad') || n.includes('badvrelse'))) {
      if (startVal > 0) pushExtra('badeværelse', { name: raw, kind: 'fixed', amount: startVal });
      if (m2Val > 0) pushExtra('badeværelse', { name: raw + ' (pr. m²)', kind: 'per_m2', amount: m2Val });
      continue;
    }
    if (n.includes('placering') && (n.includes('kkken') || n.includes('kkken'))) {
      if (startVal > 0) pushExtra('køkken', { name: raw, kind: 'fixed', amount: startVal });
      if (m2Val > 0) pushExtra('køkken', { name: raw + ' (pr. m²)', kind: 'per_m2', amount: m2Val });
      continue;
    }

    // Elektriker: ny tavle + other additive lines
    if (n.includes('tavle')) {
      if (startVal > 0) pushExtra('elektriker', { name: raw, kind: 'fixed', amount: startVal });
      if (m2Val > 0) pushExtra('elektriker', { name: raw + ' (pr. m²)', kind: 'per_m2', amount: m2Val });
      continue;
    }
    ...
    if (n.includes('tillg') || n.includes('tillaeg')) {
      ...
      if (n.includes('nyt')) {
        ...
        if (val > 0) pushExtra('døre og vinduer', { name: raw, kind: 'fixed', amount: val });
      }
    ...
    if (n.includes('trappe')) {
      if (startVal > 0) pushExtra('terrasse', { name: raw, kind: 'fixed', amount: startVal });
      if (m2Val > 0) pushExtra('terrasse', { name: raw + ' (pr. m²)', kind: 'per_m2', amount: m2Val });
      continue;
    }

    // Gulv: tillæg gulvvarme
    if (n.includes('gulv') && n.includes('varme')) {
      if (startVal > 0) pushExtra('gulv', { name: raw, kind: 'fixed', amount: startVal });
      if (m2Val > 0) pushExtra('gulv', { name: raw + ' (pr. m²)', kind: 'per_m2', amount: m2Val });
      continue;
    }

    // Tag: additive lines such as efterisolering, undertag, kviste (fixed per piece) when given as fixed/m2
    if (n.includes('efterisolering') || n.includes('undertag') || n.includes('kvist') || n.includes('kviste')) {
      if (startVal > 0) pushExtra('tag', { name: raw, kind: 'fixed', amount: startVal });
      if (m2Val > 0) pushExtra('tag', { name: raw + ' (pr. m²)', kind: 'per_m2', amount: m2Val });
      continue;
    }
  }
  ...
  fs.writeFileSync(OUT, JSON.stringify({ base: outBase, extras: outExtras }, null, 2), 'utf8');
